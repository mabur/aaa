...
Iterator max_element(Iterator first, Iterator last, Key key) {
    if (first == last) {
        return first;
    }
    auto result = first;
...
typename Container::iterator max_element(Container& container, Key key) {
    using std::begin;
    using std::end;
    return aaa::max_element(begin(container), end(container), key);
}

template<typename Container, typename Key, check_key<Key, value_type<Container>> = nullptr>
typename Container::const_iterator max_element(const Container& container, Key key) {
    using std::begin;
    using std::end;
    return aaa::max_element(begin(container), end(container), key);
}
